...
await run("email thread contact linking is driven by sender email and can return to the email", () => {
  ...
  assert.match(source, /onEditMethod\?: \(methodId: string\) => void/);
  ...
  assert.match(source, /onEditMethod=\{\(methodId\) => setContactMethodEditingId/);
  assert.match(source, /新增联系方式/);
  assert.match(source, /setContactMethodEditingId\(`method-\$\{Date\.now\(\)\}-/);
