...
await run("email thread contact linking is driven by sender email and can return to the email", () => {
  ...
  assert.match(source, /type QuickActionHeaderAction = \{/);
  ...
  assert.match(source, /headerAction=\{onEdit \? \{ label: editing \?/);
  ...
  assert.match(source, /async function startWhatsAppFollowUp\(record: CrmRecord, method: ContactMethodDraft\)/);
  assert.match(source, /requestPrompt\(\{[\s\S]*WhatsApp[\s\S]*confirmLabel: ".*?WhatsApp"/);
  assert.match(source, /purpose: "draft"[\s\S]*Generate one concise WhatsApp opening message/);
  assert.match(source, /createRecordActivity\(\{[\s\S]*title: `WhatsApp/);
  assert.match(source, /window\.open\(`\$\{whatsappUrl\}\$\{separator\}text=\$\{encodeURIComponent\(messageText \|\| trimmedPrompt\)\}`/);
  ...
  assert.match(source, /selectedRecord\.objectKey === "contacts" && \(contactMethodsEditing \|\| selectedRecordQuickContactMethods\.length === 0\)/);
  assert.match(source, /onEdit=\{selectedRecord\.objectKey === "contacts" \? \(\) => setContactMethodsEditing/);
