...
- The compose panel shows AI-assisted draft provenance before send, including generated time, source count, source links, and an explicit note that AI provenance will be preserved with the outbound message.
- The compose UI shows an AI-assisted draft marker before send, lets the user clear that marker, and automatically removes AI provenance when the user manually changes the mailbox account, linked CRM record, recipients, subject, or body before sending. It also clears stale provenance when the draft context changes through mailbox account auto-fill, newly created sender accounts, selected thread changes, or thread-to-record relinking.
- The email AI panel shows an operator-facing policy summary with enabled automatic job count, source-reference policy, history window, active knowledge coverage, and context budget. When automatic jobs are enabled, it also reminds admins that only committed `received` inbound and `sent` outbound messages enter automatic AI context.
...
- Set `EMAIL_SYNC_USER_ID` to the admin user that should own the audit context. If the configured user is unavailable, the script falls back to the first active user with `crm.admin`; passing `-- --user-id <admin-user-id>` is strict and fails if that user cannot be used. Email diagnostics and `/api/health` expose `syncScheduler.configuredUserId`, `userIdSource`, and `fallbackToAdmin` so operators can see the preferred identity and fallback policy without opening the script.
- `npm run email:verify` prints email subsystem diagnostics and a manual real-mailbox verification checklist. Its JSON output includes `operationalUser.requestedUserId`, `resolvedUserId`, `strict`, `fallbackUsed`, and `requiredPermission`, so deployment logs show whether the preferred admin id was used or an active `crm.admin` fallback was selected. Add `-- --test-connections` to test all active configured accounts against SMTP/IMAP, Gmail, or Outlook provider APIs. Add `-- --test-ai-provider` to call the configured OpenAI-compatible AI provider with bounded CRM, email, and knowledge context; the check fails unless the result reports `generationMode=provider`. Add `-- --require-live-readiness` when the command must fail unless `readiness.liveTrafficReady=true`; this mode automatically runs real mailbox tests, the provider generation check, and the application smoke check. Set `EMAIL_VERIFY_USER_ID` to choose the preferred admin user, or pass `-- --user-id <admin-user-id>` when you want strict verification against a specific user.
- `email:verify` keeps the full machine-readable result on stdout and writes a compact readiness summary to stderr with `automatedChecksOk`, `liveTrafficReady`, mailbox counts, AI provider status, smoke status, blockers, warnings, and manual actions. This keeps GitHub Actions logs readable without breaking JSON parsing.
- `npm run deploy:verify:live-email` runs the same real mailbox, AI provider, application smoke, and live-readiness gates inside the `web` container as part of deployment verification.
...
- The AI output card shows source count and a human-review badge, making it explicit that generated content is assistive and must still be reviewed before sending.
...
The email workspace shows the selected thread's compact summary and update time, with explicit badges indicating that it is used for later AI context and token reduction.
