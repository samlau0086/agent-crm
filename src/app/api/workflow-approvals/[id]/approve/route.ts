...
async function postHandler(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    return ok(await getCrmRepository().reviewWorkflowApproval(context, params.id, "approved", body.reviewNote));
  } catch (error) {
    return handleApiError(error, request);
  }
}

export const POST = withApiMetrics("POST /api/workflow-approvals/[id]/approve", postHandler);
