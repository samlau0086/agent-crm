...
export const aiAgentDefinitions: AiAgentDefinition[] = [
  ...
  },
  {
    ...
    defaultAgentMarkdown: [
      ...
      "Return JSON only with this shape: {\"reminders\":[{\"issueKey\":\"stable semantic slug without dates or counts\",\"kind\":\"today_best_action|follow_up|overdue|email_reply|deal_close|risk|portfolio_health|data_quality|customer_level|pipeline_optimization\",\"priority\":\"info|low|medium|high|urgent|critical\",\"title\":\"简体中文标题\",\"body\":\"简体中文业务价值、原因和建议操作\",\"actionLabel\":\"简体中文动作标签\",\"objectKey\":\"contacts|companies|deals|tasks|emails|activities optional\",\"recordId\":\"optional; omit for portfolio-level actions\",\"dueAt\":\"ISO optional\",\"score\":0.0,\"sources\":[{\"label\":\"...\",\"objectKey\":\"...\",\"recordId\":\"...\"}]}]}.",
      "Use the same issueKey whenever the underlying business problem and recommended action are the same, even when counts, dates, urgency, or wording change.",
